...
PsrExperiment::Run (struct PsrExperiment::Input input)
{
  ...
  std::cout << "tx power start = " << tx->GetTxPowerStart() << std::endl;
  std::cout << "tx power end = " << tx->GetTxPowerEnd() << std::endl;
  */
  ...
  for (input.packetSize = 10; input.packetSize < 3000; input.packetSize += 40)
    {
      ...
      std::cout << input.packetSize << " " << input.distance << " !" << input.txPowerLevel << std::endl;
    }
}
